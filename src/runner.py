...
from code_completion_final import Code_Completion_Baseline
...
def create_hole(tokens): #挖一个洞，返回这个洞长什么样
    hole_size =  min(randint(1, max_hole_size), len(tokens) - 1) #随机这个洞的大小
    hole_start_idx = randint(1, len(tokens) - hole_size) #随机洞的开始坐标
    prefix = tokens[0:hole_start_idx] #洞前部分
    expected = tokens[hole_start_idx:hole_start_idx + hole_size] #洞中内容
    suffix = tokens[hole_start_idx + hole_size:] #洞后部分
...
for tokens in query_token_lists:
    ...
    print("completion words:%s: "%(completion))#
    print("expected words:%s: "%(expected))#
    if(len(suffix)>0):
        print("suffix:%s: " % (suffix[0]))
    else:
        print("suffix:%s: " % ('[]'))
