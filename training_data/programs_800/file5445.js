...
;(function ($) {var _version = "jQuery.my 0.9.9";
	...
	function _putmanifest (obj0, root0) {
		...
		var root=root0||forms, obj=obj0, path, id, prev, res;
	...
	/**/	".my-form-list": function ($o,list) {
			//object is list of forms
				...
				if (n(list) && isA(list)) {
					...
					if (!eq){
						...
						if (!Object.equal(old,mod) || mod.length!=list.length) {
							...
							$c.filter("*:not(.ui-sortable-placeholder)").each(function () {
								var $x = $(this),
									md = $x.data("formlist")||{},
									hash = md.sdbmCode;
							...
							for (i=0;i<list.length;i++) {
								if (present[hash[i]]) {
									$n = present[hash[i]].detach().appendTo($o);
									result.push($n.my("data"));
								} else {
									$n = $(/\{/.test(tmpl)?tmpl.assign(list[i]):tmpl).appendTo($o);
							...
							(function () {$o.trigger(Ch);}).debounce();
							return result;
						}
					}
					return old;
				...
				function _itemChange () {
					var $this = $(this);
					if ($this.data("my")) $this.data("formlist").sdbmCode = f.sdbmCode($this.my("data"));
					$o.trigger("check.my");
				}
			},
	...
	/**/	".ui-sortable": function ($o, list) {
				...
				if (n(list) && isA(list)) {
					...
					if ($p.size()!=0) {
						...
						$c = $o.find($o.sortable("option","items")).filter(":visible:not(:disabled, .ui-state-disabled)");
				...
				"": function ($o,v) {
					if (n(v)) {
						...
						//now ditinguish between jQ selectmenu plugin and jQ Mobile
							if ($o.selectmenu("option").theme!==undefined) $o.selectmenu("refresh",true);
		...
		if (xdata) {
			...
			if (oui.bind != N) {
				...
				try {
					...
					f.con ("Error "+ e.name+" validating "+selector, $root );
				}
				var ec = p.errorCss;
				var jqec = "ui-state-error";

				try {
					if (value!=N) val = _field($we,_bind(d,value,oui,$we));
				}
				catch (e) {
					err=p.messages.formError||"Error";
				}
			...
			while (i<b.length) {
				if (isS(b[i])) {
					...
					if (/^[a-z0-9]+(#[a-z0-9\-_]+)?(\.[a-z0-9\-_]+)*$/i.test(b0)) {
						...
						if (!p["class"] && /\./.test(key)) p["class"]=(key.to(1)=="#"?key.substr(p.id.length+1):key).split(".").compact(true).join(" ");
...
/* jQuery.my.modal 0.5
 * Requires Sugar 1.4.1+, jQuery 1.11+, $.my 0.9.7+
 *
